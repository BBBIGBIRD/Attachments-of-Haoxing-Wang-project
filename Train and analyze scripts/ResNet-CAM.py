...
# 加载ResNet模型
def load_resnet_model(pth_file, num_classes, device):
    model = models.resnet18()  # 使用ResNet50，您可以根据需要选择其他版本
    model.fc = torch.nn.Linear(model.fc.in_features, num_classes)  # 修改全连接层输出
...
if __name__ == "__main__":
    pth_file = 'C://Users//15097//Desktop//训练过程结果//resnet18_classification.pth'  # 替换为你的VGG-16模型文件路径
    ...
    save_path = 'resnet_heatmap.jpg'  # 热力图保存路径
    num_classes = 11  # 修改为模型的输出类别数量
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    # 加载ResNet模型和图片
    model = load_resnet_model(pth_file, num_classes, device)
